...
By default, the name of the parameter to be passed for `Name` query field is `@Name`. If you have passed the entity object during the calls to [Update](/docs/operations/update.md) operation and that instance also contains the `Name` property, then they are colliding. See below.
