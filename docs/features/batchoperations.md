...
In RepoDB, the following operations (i.e.: [InsertAll](/docs/operations/insertall.md), [UpdateAll](/docs/operations/updateall.md) and [MergeAll](/docs/operations/mergeall.md)) are the batch operations. They are all ACID in nature.

## Identity Setting Alignment

The library has enforced an additional logic to ensure the identity setting alignment is correct if the [InsertAll](/docs/operations/insertall.md) and [MergeAll](/docs/operations/mergeall.md) operations are being used.
...
By calling the [Insert](/docs/operations/insert.md) operation below...
...
```
The default value of the `batchSize` used to be 10, but the code now calculates the maximum batchsize and from that smart batch sizes to use. So if (with systemlimits) it is possible to insert 678 records at once, we will try to insert 678 records until we have less remaining... Then we will try 'banknote' batchvalues going down, like 500, 200, 100, 50, 20, 10.. And like before
we will do everything below 10 in one batch. This will typically give you good batch handling without having to generate queries and code for each and every possible batchsize.

If you do as the previous example and pass 100, the packed-statement is cached and is being executed 10 times with 100 rows each. All the parameters will be passed into its proper indexes, depending on the number of batches. The execution is more optimal than it was in the previous section as it is executing the multiple SQL statements in one-go.
Without having the batch operations, the executions will be 1000 times. **It is typically no longer necessary to provide a batchsize yourself, as with the newer logic the optimal value is already passed.**

> In SQL Server, the ADO.NET maximum number of parameters are 2100. RepoDb is smart enough to use that knowledge in its auto-batching, and in limiting the value to not cause errors.

## Behind the scene of the Batch Operations

When you call any of the push batch operations (i.e.: [InsertAll](/docs/operations/insertall.md), [UpdateAll](/docs/operations/updateall.md) or [MergeAll](/docs/operations/mergeall.md)), the following activities are happening behind the scene.
