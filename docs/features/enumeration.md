...
> If you call any model-based operations (i.e.: [Query](/docs/operations/query.md), [Update](/docs/operations/update.md), [Merge](/docs/operations/merge.md), etc), the default conversion is not used as it has already projected the correct database type to be passed/parsed based the underlying table schema.
...
As type inference is supported by the library, you can as well infer the enumeration directly when fetching a row from the database via [ExecuteQuery](/docs/operations/executequery.md).
