...
For [BulkInsert](/docs/operations/bulkinsert.md) operation, it simply calls the [WriteToServer()](https://learn.microsoft.com/en-us/dotnet/api/system.data.sqlclient.sqlbulkcopy.writetoserver?view=dotnet-plat-ext-6.0) method to bring all the data into the database. Unless you would like to bring the newly generated identity column values back to the application after the execution, there is no additional logic is implied.

Image below shows the data flow of the [BulkInsert](/docs/operations/bulkinsert.md) operation.

<img src="../../assets/images/site/bulkinsert.svg" />

For the [BulkDelete](/docs/operations/bulkdelete.md), [BulkMerge](/docs/operations/bulkmerge.md) and [BulkUpdate](/docs/operations/bulkupdate.md) operations, an implied logic and technique has been utilized.

Image below shows the data flow of the [BulkMerge](/docs/operations/bulkmerge.md) operation.

<img src="../../assets/images/site/bulkmerge.svg" />

Basically, a pseudo-temporary table will be created in the database under a transaction context. It then uses the [BulkInsert](/docs/operations/bulkinsert.md) operation to target that pseudo-temporary table and process the data afterwards.
...
For the other bulk operation methods (i.e.: [BulkDelete](/docs/operations/bulkdelete.md), [BulkMerge](/docs/operations/bulkmerge.md) and [BulkUpdate](/docs/operations/bulkupdate.md)), the execution can be maximized by targeting the underlying table indexes (via qualifiers). To do this, simply pass a list of [Field](/class/field) object when calling the operations.
...
The arguments `qualifiers`, `isReturnIdentity` and `usePhysicalPseudoTempTable` were provided to the [BulkDelete](/docs/operations/bulkdelete.md), [BulkMerge](/docs/operations/bulkmerge.md) and [BulkUpdate](/docs/operations/bulkupdate.md) operations.
...
The library has enforced an additional logic to ensure the identity setting alignment if the `isReturnIdentity` is enabled during the calls. This affects both the [BulkInsert](/docs/operations/bulkinsert.md) and [BulkMerge](/docs/operations/bulkmerge.md) operations.
...
The library is automatically setting the value of the `options` argument to [SqlBulkCopyOptions.KeepIdentity](https://learn.microsoft.com/en-us/dotnet/api/system.data.sqlclient.sqlbulkcopyoptions?view=dotnet-plat-ext-6.0) when calling the [BulkDelete](/docs/operations/bulkdelete.md), [BulkMerge](/docs/operations/bulkmerge.md) and [BulkUpdate](/docs/operations/bulkupdate.md) if you have not passed any qualifiers and if your table has an identity key column.

In addition, when calling the [BulkDelete](/docs/operations/bulkdelete.md), [BulkMerge](/docs/operations/bulkmerge.md) and [BulkUpdate](/docs/operations/bulkupdate.md) operations, the library is creating a pseudo-temporary table behind the scene. It requires your user to have the proper CREATE TABLE privilege to create a table in the database, otherwise a [SqlException](https://learn.microsoft.com/en-us/dotnet/api/system.data.sqlclient.sqlexception?view=dotnet-plat-ext-6.0) will be thrown.

---

## BulkDelete

The code snippets below only showcasing the [BulkDelete](/docs/operations/bulkdelete.md) via [IEnumerable<T>](https://learn.microsoft.com/en-us/dotnet/api/system.collections.generic.ienumerable-1?view=net-7.0).
...
The code snippets below only showcasing the [BulkInsert](/docs/operations/bulkinsert.md) via [IEnumerable<T>](https://learn.microsoft.com/en-us/dotnet/api/system.collections.generic.ienumerable-1?view=net-7.0).
...
The code snippets below only showcasing the [BulkMerge](/docs/operations/bulkmerge.md) via [IEnumerable<T>](https://learn.microsoft.com/en-us/dotnet/api/system.collections.generic.ienumerable-1?view=net-7.0).
...
The code snippets below only showcasing the [BulkUpdate](/docs/operations/bulkupdate.md) via [IEnumerable<T>](https://learn.microsoft.com/en-us/dotnet/api/system.collections.generic.ienumerable-1?view=net-7.0).
