...
A statement builder is an object that is being used by the library to generate the SQL statements for all operations. To be more practical, when you call the [QueryAll](/docs/operations/queryall.md) operation, a SQL statement is being constructed via statement builder and is being executed by the engine against the database.
...
| [Average Type Resolver](/extensibility/averagetyperesolver) | A resolver that is being used to define which type of .NET CLR type the library is going to be used when calling the [Average](/docs/operations/average.md) and [AverageAll](/docs/operations/averageall.md) operations.|
...
| Name | Description |
|:-----|:------------|
| CreateBatchQuery | A method that generates a SQL statement for [BatchQuery](/docs/operations/batchquery.md) operation. |
| CreateMerge | A method that generates a SQL statement for [Merge](/docs/operations/merge.md) operation.|
| CreateMergeAll | A method that generates a SQL statement for [MergeAll](/docs/operations/mergeall.md) operation. |

### Why?

| [BatchQuery](/docs/operations/batchquery.md) | Each RDBMS data provider has a different syntax for paging/batching. |
| [Merging](/docs/operations/merge.md) | Not all RDBMS data providers are supporting the default merge statement. |

Below is the sample code to implement the [Merge](/docs/operations/merge.md) operation.
...
It is not necessary to override a virtual method of the [BaseStatementBuilder](/class/basestatementbuilder). However, if you do override, then below is a sample referrence-code that overrides the default implementation of [CreateTruncate](/docs/operations/truncate.md) operation.
